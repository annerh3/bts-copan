...
interface GetPermissionsParams {
  ...
  p_order_by: OrderByColumn
  p_order_dir: "asc" | "desc"
  p_parent_phone: string | null
}

// función para obtener los permisos de forma paginada
export const getPermissionsPage = async (params: GetPermissionsParams) => {
  try {
    const { data, error } = await supabase.rpc("get_permissions_paginated", params)
    ...
    // si no hay datos, retornar un objeto con items vacios
    return {
      items: [],
      total: 0,
      page: params.p_page,
      per_page: params.p_per_page,
    }
  } catch (error) {
